...
import numpy as np
from helper_functions_charuco import cv_find_chessboard, get_chessboard_points_3D, get_depth_at_pixel, convert_depth_pixel_to_metric_coordinate
...
class PoseEstimation:
	...
	def __init__(self, frames, intrinsic, charuco_params):
		self.frames = frames
		self.intrinsic = intrinsic
		self.charuco_params = charuco_params
	...
	def get_chessboard_corners_in3d(self):
		...
		for (serial, frameset) in self.frames.items():
			...
			list_found_corners, IDs = cv_find_chessboard(depth_frame, infrared_frame, self.charuco_params)
			corners3D[serial] = [IDs, None, None, None]
			if list_found_corners:
				found_corners = list_found_corners[0]
				points3D = np.zeros((3, self.charuco_params.chessboardCorners.shape[0]))
				validPoints = [False] * self.charuco_params.chessboardCorners.shape[0]
	...
	def perform_pose_estimation(self):
		...
		retval = {}
		for (serial, [IDs, points2D, points3D, validPoints] ) in corners3D.items():
			objectpoints = get_chessboard_points_3D(self.charuco_params)
	...
	def find_chessboard_boundary_for_depth_image(self):
		...
		for (serial, frameset) in self.frames.items():
			...
			depth_frame = post_process_depth_frame(frameset[rs.stream.depth])
			infrared_frame = frameset[(rs.stream.infrared, 1)]
			found_corners, points2D = cv_find_chessboard(depth_frame, infrared_frame, self.charuco_params)
