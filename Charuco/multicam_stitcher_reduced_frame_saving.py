...
from stitcher_functions import get_transformation_matrix, get_charuco_points, get_3d_points, get_transform_short
            ...
            for idx, from_device in enumerate(device_list[:-1]):
                for to_device in device_list[idx + 1:]:
                    if to_device != from_device:
                        chessboard_points[from_device][to_device] = get_3d_points(frames, [from_device, to_device], intrinsics_devices, charuco_board)

        for idx, from_device in enumerate(device_list[:-1]):
            for to_device in device_list[idx + 1:]:
                if to_device != from_device:
                    temp_transform, temp_rms = get_transform_short(chessboard_points[from_device][to_device][1], chessboard_points[from_device][to_device][0])
