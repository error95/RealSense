...
def get_chessboard_points_3D(chessboard_params):
	...
	return np.transpose(chessboard_params.chessboardCorners)


def cv_find_chessboard(depth_frame, infrared_frame, aruco_dict):
	"""
	...
	corners, ids, rejectedImgPoints = cv2.aruco.detectMarkers(infrared_image, aruco_dict.dictionary)
	...
	if len(corners) > 0:
		...
		for corner in corners:
			cv2.cornerSubPix(infrared_image, corner,
							 winSize=(3, 3),
							 zeroZone=(-1, -1),
							 criteria=criteria)
		res2 = cv2.aruco.interpolateCornersCharuco(corners, ids, infrared_image, aruco_dict)
...
def cv_find_chessboard_np(depth_frame, infrared_image, aruco_dict):
	"""
	Searches the chessboard corners using the set infrared image and the
	checkerboard size
...
def least_error_transfroms(graph, rms):
	"""
		Gets the calculated transforms from all calculated cameras and their RMS error
		and returns the transformation matrices from all cameras to first camera in the list with least summed RMS error
		if possible. Method used is Dijkstra's shortest path with RMS error as edge weight

		Parameters:
		-----------
		graph 	 	 : dictionary containing all calculated transforms between cameras

		rms      :  dictionary containing the rms error in transforms

		Returns
		-------
		matrices: dictionary if paths to all cameras are found
			{N*[4,4]} matrix with N being the number of cameras and serial number used for indexing

		else: returns 0 indicating that not all cameras are found
		"""
	# Setting up dijkstra's algorithm to find shortest path
	path = {}
	idx = 0
	first_camera = list(graph)[0]

	for device in graph.keys():
		path[device] = {}
		path[device][first_camera] = {'dist': np.inf, 'prev': 'undef', 'used': False}

		for device2 in graph.keys():
			if idx == 0:
				path[device][device2] = {'dist': 0, 'prev': 'undef', 'used': False}
				idx = 1
			if rms[device][device2] != np.inf:
				path[device][device2] = {'dist': np.inf, 'prev': device2, 'used': False}

	# Calculating shortest path
	stop = False
	while not stop:
		current_min = np.inf
		for key_iter in path:
			if path[key_iter][first_camera]['dist'] <= current_min and not path[key_iter][first_camera]['used']:
				path[key_iter][first_camera]['used'] = True
				current_min = path[key_iter][first_camera]['dist']
				from_camera = key_iter

		for neighbours in path[from_camera]:
			if rms[neighbours][from_camera] != np.inf:
				new_error = path[from_camera][first_camera]['dist'] + rms[neighbours][from_camera]
				if new_error <= path[neighbours][first_camera]['dist']:
					path[neighbours][first_camera]['dist'] = new_error
					path[neighbours][first_camera]['prev'] = from_camera

		stop = all_used(path)
		if stop:
			print("STOP")

	# Calculating transformation matrices for paths found
	matrices = {}
	calibrated_cameras = 1
	for camera in graph.keys():
		start_camera = camera
		if first_camera in path[camera]:
			the_previous = path[camera][first_camera]['prev']
			full_matrix = np.identity(4)
			calibrated = 0
			while the_previous != 'undef':
				calibrated = 1
				full_matrix = np.matmul(graph[start_camera][the_previous].pose_mat, full_matrix,)
				start_camera = the_previous
				the_previous = path[the_previous][first_camera]['prev']
			matrices[camera] = full_matrix
			calibrated_cameras += calibrated
	if calibrated_cameras == len(graph):
		return matrices
	else:
		return 0
...
def all_used(path):
	first_camera = list(path)[0]
	"""
		Checks if all vertices are used

		Parameters:
		-----------
		path: dictionary containing all edges

		Return:
		----------
		is_used_up: bool signaling if all vertices are used

		"""
	is_used_up = True
	for key_iter in path:
		if not path[key_iter][first_camera]['used']:
			is_used_up = False
	return is_used_up


def matrix_viewer(rms):
	key_list = rms.keys()
	first_cam = 0
	rms_matrix = np.full((len(key_list), len(key_list)), np.inf)
	for from_device in key_list:
		second_cam = 0
		for to_device in key_list:

			if rms[from_device][to_device] != np.inf:
				rms_matrix[first_cam][second_cam] = rms[from_device][to_device]
			second_cam += 1
		first_cam += 1
	return rms_matrix
